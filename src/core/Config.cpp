...
namespace YG {
    ...
    String Config::GenerateContent() const {
        ...
        stream << KEY_WINDOW_WIDTH << L"=" << GetString(KEY_WINDOW_WIDTH) << L"\n";
        stream << KEY_WINDOW_HEIGHT << L"=" << GetString(KEY_WINDOW_HEIGHT) << L"\n";
        stream << KEY_WINDOW_X << L"=" << GetString(KEY_WINDOW_X) << L"\n";
        stream << KEY_WINDOW_Y << L"=" << GetString(KEY_WINDOW_Y) << L"\n";
        stream << KEY_WINDOW_MAXIMIZED << L"=" << GetString(KEY_WINDOW_MAXIMIZED) << L"\n";
        stream << L"\n";
        
        // 功能设置
        stream << L"# Feature Settings\n";
        stream << KEY_AUTO_REFRESH << L"=" << GetString(KEY_AUTO_REFRESH) << L"\n";
        stream << KEY_SHOW_SYSTEM_COMPONENTS << L"=" << GetString(KEY_SHOW_SYSTEM_COMPONENTS) << L"\n";
        stream << KEY_CONFIRM_UNINSTALL << L"=" << GetString(KEY_CONFIRM_UNINSTALL) << L"\n";
        stream << KEY_DEEP_SCAN << L"=" << GetString(KEY_DEEP_SCAN) << L"\n";
        stream << L"\n";
        
        // 日志设置
        stream << L"# Log Settings\n";
        stream << KEY_LOG_LEVEL << L"=" << GetString(KEY_LOG_LEVEL) << L"\n";
        stream << L"\n";
        
        // 语言设置
        stream << L"# Language Settings\n";
        stream << KEY_LANGUAGE << L"=" << GetString(KEY_LANGUAGE) << L"\n";
        stream << L"\n";
        ...
        for (const auto& pair : m_values) {
            // 跳过已经写入的标准配置
            if (pair.first == KEY_WINDOW_WIDTH || pair.first == KEY_WINDOW_HEIGHT ||
                pair.first == KEY_WINDOW_X || pair.first == KEY_WINDOW_Y ||
                pair.first == KEY_WINDOW_MAXIMIZED || pair.first == KEY_AUTO_REFRESH ||
                pair.first == KEY_SHOW_SYSTEM_COMPONENTS || pair.first == KEY_CONFIRM_UNINSTALL ||
                pair.first == KEY_DEEP_SCAN || pair.first == KEY_LOG_LEVEL ||
                pair.first == KEY_LANGUAGE) {
                continue;
            }
